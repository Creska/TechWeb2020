...
var example = {
	...
	"quests": [
		{	
			...
			"activities": [
				{
					"activity_text": "<div id='activity0' class='activity'> <p>Questa è la prima attività.<br>Clicca su PROSEGUI per andare alla seconda.</p> <button id='activity0answerfield' class='answerfield' onclick='if (story.settings.optionchoice1) goToActivity(1); else goToActivity(2);'>Prosegui</button> </div>",
					"right_answer": "",
					"need_human_evaluation": false,
					"action_on_activity_answer": "goToActivity(1);" // ignorare questo campo
				},
				{
					"activity_text": "<div id='activity1' class='activity'> <p>Questa è la seconda attività. Avevi scelto la prima opzione.<br>Clicca su FINE per completare il test.</p> <button id='activity1answerfield' class='answerfield' onclick='window.alert(\"Test andato a buon fine\");'>Fine</button> </div>",
					"right_answer": "",
					"need_human_evaluation": false,
					"action_on_activity_answer": "window.alert('Test andato a buon fine');" // ignorare questo campo
				},
				{
					"activity_text": "<div id='activity2' class='activity'> <p>Questa è la terza attività. Avevi scelto la seconda opzione.<br>Clicca su FINE per completare il test.</p> <button id='activity1answerfield' class='answerfield' onclick='window.alert(\"Test andato a buon fine\");'>Fine</button> </div>",
					"right_answer": "",
					"need_human_evaluation": false,
					"action_on_activity_answer": "window.alert('Test andato a buon fine');" // ignorare questo campo
				}
...
$(function () {
	...
	function goToActivity(activity_n) {
		if (activity_n) {
			$("#activity" + String(currentStatus.currentActivity)).replaceWith(story.quests[currentStatus.currentQuest].activities[activity_n].activity_text);
		}
		else {
			$("#quest" + String(currentStatus.currentQuest)).append(story.quests[currentStatus.currentQuest].activities[activity_n].activity_text);
		}
